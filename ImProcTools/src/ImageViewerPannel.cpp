...
ImageViewerPannel::ImageViewerPannel()
{
...
DrawingInformations GetImageDrawingInfo(Walnut::Image* image)
{
	...
	if (image)
	{
		...
		//auto image_center = ImVec2{ window_center.x + offset.x ,window_center.y + offset.y  };
		//auto image_center = window_center;

		p0 = ImVec2((window_center.x - (image->GetWidth() / 2)), (window_center.y) - (image->GetHeight() / 2));
		...
		auto zoom_function = [](float zoom_value) { return std::exp(0.5f * zoom_value - 3.0f); };

		auto finalp0 = ImVec2{ window_center.x + t0.x * zoom_function(zoom) + offset.x,window_center.y + t0.y * zoom_function(zoom) + offset.y };
		auto finalp1 = ImVec2{ window_center.x + t1.x * zoom_function(zoom) + offset.x,window_center.y + t1.y * zoom_function(zoom) + offset.y };

		auto image_center = ImVec2{ finalp0.x + image->GetWidth() / 2 ,finalp1.y + image->GetHeight() / 2 };
		draw_info = DrawingInformations{ pos0,pos1,finalp0,finalp1 };
	}
	return draw_info;
}
void DrawDragabbleImage(Walnut::Image* image, DrawingInformations info, bool bDebug = false)
{
	if (image)
	{
		...
		if (bDebug)
		{
			//draw_list->AddCircleFilled(window_center, 5.0f, IM_COL32(220, 30, 25, 255));
			//draw_list->AddCircleFilled(image_center, 5.0f, IM_COL32(30, 30, 250, 255));
			//draw_list->AddLine(window_center, { t0.x - window_center.x,t0.y - window_center.y }, IM_COL32(20, 50, 250, 255));
			//draw_list->AddLine(window_center, { t1.x - window_center.x,t1.y - window_center.y }, IM_COL32(20, 50, 250, 255));
			//draw_list->AddCircleFilled(p0, 5.0f, IM_COL32(30, 150, 25, 255));
			//draw_list->AddCircleFilled(p1, 5.0f, IM_COL32(30, 150, 25, 255));
		}
		ImGui::PopClipRect();
	}
}
